...
export function UserProfileDropdown({ onOpenSettings }: UserProfileDropdownProps) {
  ...
  const firstLetter = user?.name?.charAt(0)?.toLowerCase() || "u"
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent
        align="start"
        className="w-64 p-0 rounded-xl border border-border bg-popover shadow-lg"
      >
        {/* User Info Section */}
        <div className="p-4 pb-3">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-foreground">{user?.name}</span>
            <span className="text-xs px-2 py-0.5 rounded bg-muted text-muted-foreground font-light tracking-wide">
              {user?.plan === "free" ? "FREE" : user?.plan === "pro" ? "PRO" : "ENT"}
            </span>
          </div>
        ...
        <div className="p-2">
          ...
          <MenuItem
            ...
            label="前往官网"
            onClick={() => window.open("https://flowith.io", "_blank")}
          />
          <MenuItem
        ...
        <div className="p-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <SocialIcon href="https://discord.com" icon="discord" />
